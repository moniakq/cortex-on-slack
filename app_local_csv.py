...
def handle_message_events(ack, body, say):
    try:
        ...
        show_sql = False
        send_as_file = False # New flag for sending a file
        # 2. Check for the /sql command
        if prompt.strip().endswith("/sql"):
            show_sql = True
            prompt = prompt.strip()[:-4].strip()
        # 3. Check for the /file command
        if prompt.strip().lower().endswith("/file"):
            send_as_file = True
            prompt = prompt.strip()[:-5].strip()
        response = ask_agent(prompt)
        # print(prompt)
        # print(response['text'])
        # print(response['sql'])
        display_agent_response(response, say, channel_id, show_sql, send_as_file, prompt)
...
def display_agent_response(content, say, channel_id, show_sql, send_as_file, prompt):
    ...
    if content.get('sql'):
        ...
        if send_as_file:
            if df.empty:
                say(text=f"Your query ran successfully but returned no data to create a file.\n\n*Query:*\n```\n{sql}\n```")
            else:
                try:
                    csv_file_name = "cortex_results.csv"
                    df.to_csv(csv_file_name, index=False) # Save df to a local CSV file
                    # Upload the file to the specific channel
                    app.client.files_upload_v2(
                        channel=channel_id,
                        file=csv_file_name,
                        title="Cortex Analyst Results",
                        initial_comment=f"As requested, here are the results for your query: {prompt}, attached as a CSV file."
                    )
                except Exception as e:
                    say(text=f"Sorry, I ran into an error while creating or uploading the CSV file: {e}")
            return
